...
`extract()` cleans text by default, but can be disabled by setting the parameter `do_clean_text` to false. Cleaning standardizes spacing, punctuation, and capitalization, which helps regex pattern matching.
