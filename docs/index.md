...
User-supplied patterns allow
[`extract()`](https://judgelord.github.io/regextable/reference/extract.md)
to match text in any dataset or context.
...
is applied to standardize text. It lowercases text, removes excess
punctuation, replaces line breaks and dashes with spaces, and collapses
multiple spaces into a single space. Text cleaning is applied only
during matching and does not modify the original input data. Users can
disable this behavior by setting `do_clean_text = FALSE`.
...
table. For each input row, it returns at most one match (the first
matched pattern), along with the corresponding pattern and optional
metadata from the pattern table.
