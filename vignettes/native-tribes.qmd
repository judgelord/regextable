...
This example demonstrates how to use `regextable::extract()` to extract Native American Tribe names in the Tribal Directory data (`tribes_df`). It applies the tribe regex patterns in `tribes_regex` from the "Native American Tribes/Groups Regex Table" sheet to the `Tribe` column and returns the matched tribe names along with their source reference.
